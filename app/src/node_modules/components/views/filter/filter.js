...
module.exports = React.createClass({
    ...
    return {
      targets : {
        actions : [
          ...
          {by:'target-date-after',type:'date',attr:'targetdate', relation:'after'},
          {by:'target-date-before',type:'date',attr:'targetdate', relation:'before'}
        ],
        recommendations : [
          {by:'issue',type:'entities',attr:'issues',relation:'to'},
          {by:'group',type:'entities',attr:'groups',relation:'to'},
      ...
      _.each(filters,function(filter,key){
        ...
        if (typeof targetFilter !== 'undefined') {
          ...
          if (targetFilter.type === 'entities') {
            ...
            if (targetFilter.relation === 'after') {
              conditions.push(this.dateAfterCondition(targetFilter,filter))
            } else if (targetFilter.relation === 'before') {
              conditions.push(this.dateBeforeCondition(targetFilter,filter))
            }

          } else {         
              conditions.push(this.textCondition(targetFilter,filter))
          }
        } else {
          conditions.push(function(entity){      
            return entity[key] === filter
          })
        }
      },this);
  ...
  entitiesCondition : function(targetFilter,filter){
    var flux = this.getFlux()
  ...
  textCondition : function(targetFilter,filterItem){
    return function(entity){      
      return entity[targetFilter.attr] === filterItem
    }
  },
  dateAfterCondition : function(targetFilter,filterItem){
    return function(entity){      
      return (entity[targetFilter.attr] !== '' || entity[targetFilter.attr].format() === 'Invalid date') 
           && entity[targetFilter.attr].isAfter(moment(filterItem,'MM-YYYY',true),'month')
    }
  },
  dateBeforeCondition : function(targetFilter,filterItem){
    return function(entity){      
      return (entity[targetFilter.attr] !== '' || entity[targetFilter.attr].format() === 'Invalid date') 
           && entity[targetFilter.attr].isBefore(moment(filterItem,'MM-YYYY',true),'month')
    }
  },
  renderMain : function (){
    var flux = this.getFlux()
