...
module.exports = React.createClass({
    ...
    return (
      [
        <Row className="header-row">
          <Col md={10}>
            <h2 className="header-type-large">{'Action - ' + entity.id}</h2>
        ...
        <Row className="content-main">
          <Col md={8} className="column-smart">
          ...
          <Col md={4} className="column-categories">  
  ...
  renderSMART : function(entity) {
    return ([
      <h5 className='tt-none'>SMART criteria: Assessment coming soon</h5>,
      <a href="#" title="About S.M.A.R.T. criteria" onClick={this.handleNavClick('smart')} className='smart-information'>
        <span className="icon-information"/>
      </a>,
      <ul className="list-inline list-bordered" style={{width:'100%'}}>
  ...
  renderIssues : function (entity){
    ...
    if (items.length === 0){
      ...
      return ([
        <h6>{items.length === 1 ? "Issue" : "Issues" }</h6>,
        <CategoryList
          entities = {items} />
      ])
    }
  ...
  renderGroups : function (entity){
    ...
    if (items.length === 0){
      ...
      return ([
        <h6>{items.length === 1 ? "Population Group" : "Population Groups" }</h6>,
        <CategoryList
          entities = {items} />
      ])
    }
  ...
  renderAgencies : function (entity){
    ...
    if (items.length === 0){
      ...
      return ([
        <h6>{items.length === 1 ? "Responsible Agency" : "Responsible Agencies" }</h6>,
        <CategoryList
          entities = {items} />
      ])
    }
  ...
  renderRecs : function (entity){
    ...
    if (recs.length === 0 ) {
      ...
      return ([
        <h2>{recs.length + ((recs.length === 1) ? ' Related UPR Recommendation' : ' Related UPR Recommendations')}</h2>,
        <EntityTable
