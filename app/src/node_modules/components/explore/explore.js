      ...
      if (category === 'issues') {
        ...
        if (!this.state.issuesLoading) {
          entities = _.map(flux.store(category).entities(),function(entity){
            ...
            entity.countActions = flux.store('actions').countWhere({issue:entity.id})
            entity.countRecs = flux.store('recommendations').countWhere({issue:entity.id})        
            entity.countRecsAccepted = flux.store('recommendations').countWhere({issue:entity.id,response:'Accepted'})        
            return entity
        ...
        if (!this.state.agenciesLoading) {          
          entities = _.map(flux.store(category).entities(),function(entity){
            entity.countActions = flux.store('actions').countWhere(function(item){
              return item.agency.split(',').indexOf(entity.id) > -1
            })     
            entity.countRecommendations = 0
            return entity
          })
        }
      }
