//react 
var React         = require('react')

//helpers
var _         = require('lodash')
var log       = require('debug')('src:components:select-taxonomy')


module.exports = React.createClass({

  render: function () {
    log('props', this.props)

    return (      
      <ul className="list-inline nav-primary-list" role="presentation">
        {
          _.map(this.props.items,function (item, i, items) {
            var width = 1/items.length * 100 + '%'
            return (
              <li key={i} className={item.active ? 'nav-primary-li active' : 'nav-primary-li'}  style={{width:width}} role="presentation">
                <a  
                    role="button" 
                    title={'Explore by ' + item.title}
                    className='btn'
                    href='#'
                    onClick={this.handleItemClick(item.slug)}>
                  <span className={'icon-'+ item.slug}></span>
                  <div className='title'>{item.title}</div>
                </a>
                <div className="arrow" style={{left: "50%"}}></div>
              </li>
            )
          },this)
        }               
      </ul>
    )
  },

  handleItemClick: function (slug) {
    return _.bind(function (e) {
      e.preventDefault()
      this.props.flux.actions[this.props.type](slug)
    },this)
  }
})
