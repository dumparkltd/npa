...
var TableLinkRow = React.createClass({
  ...
  render: function () {    
    ...
    return (
        ...
        <tr className={className} onClick={(linked) ? this.handleRowClick(entity.id,this.props.entityType) : false} >
          {
            _.map(this.props.cols,function(col,i){
              return (
                <td key={i} className={'hrc-cell hrc-cell-'+col.type}>{this.renderCell(entity,col)}</td>
              )
            },this)
          }
        </tr>
