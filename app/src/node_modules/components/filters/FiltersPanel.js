      ...
      if (!this.state.showFilters){
        return (
          <Grid className='hrc-filter-panel-wrapper hrc-filter-inactive' fluid={true}>
                <a href='#' className='hrc-filter-toggle pull-right' onClick={this.toggleFilters()}>Show filters <i className='icon-filter'/></a>
        ...
        return (
          <Grid className='hrc-filter-panel-wrapper' fluid={true}>
            <a href='#' className='hrc-filter-toggle pull-right' onClick={this.toggleFilters()}>Hide filters <i className='icon-filter'/></a>
            <Grid className='hrc-filter-panel hrc-filter-active'>
              <Row className='hrc-filter-wrapper'>
                <Col sm={11}>
                  <Row>
                    {
                      _.map(this.props.filtergroups, function(group){
                        ...
                        return (
                          <Col className='hrc-filter-group' sm={group.span}>
                            {group.title !== '' ? (<h5>{group.title}</h5>) : null}
                            {groupFiltered ? (<a className='hrc-filter-group-reset pull-right' href='#' onClick={this.handleGroupReset(group)}>Reset</a>) : null}
                            {
                              _.map(group.filters,function(filter,i){
                                return (
                                  <span
                                    className="hrc-filter-component"
        ...
        component = (
          <span className='filter-component-icon'>
            <a href='#' onClick={this.handleIconClick(filter.by, nextOption.value)}>
              <span role="img" className={filter.icons[currentIndex]}></span>
              <div>{currentOption.value}</div>
              <div>{currentOption.prefix + filter.label}</div>
            </a>
          </span>
        )
