          ...
          <Grid className={'hrc-filter-panel ' + (this.state.showAdvancedFilters ? 'show-advanced-filters' : 'hide-advanced-filters')}>
            {
              _.map(filterRows,function(row,i){
                ...
                return (
                  ...
                  <Row className={hasTitle ? 'row-with-title' : ''} key={i}>                     
                    {rowtitle}
                    
                    {
                      _.map(row.groups, function(group, i){                          
                        ...
                        return (
                          <Col className={'hrc-filter-group hrc-filter-group-'+ group.group + ' col-xxs-' + group.span.xxs} xs={group.span.xs} sm={group.span.sm} key={i}>
                            {
                              group.title !== '' ? (
                                <h5>
                                  {group.title}
      ...
      case 'multiselect' :
        ...
        var TagItem = React.createClass({
          render : function() {
            ...
            return (
              <span>{ title.length > 20 ? title.substr(0,20)+'...' : title }</span>
            )
          }
        }) 
