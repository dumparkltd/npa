...
var log         = require('debug')('src:components:filtersPanel')
...
var FiltersPanel = React.createClass({
  ...
  render: function () {
    ...
    if (this.state.loading[this.props.type]) {
      ...
      log(_.keys(this.props.query).length + ' filters active')
      ...
      return (
        <div className='hrc-filter-panel-wrapper'>
          ...
          <Grid>
            ...
            {
              (!this.state.showAdvancedFilters && this.noOfHiddenActiveFilters() > 0)
                ? (<span className="hidden-xs">{ '(' + this.noOfHiddenActiveFilters() + ' active)'}</span>) 
                : null
            }
            {
              (!this.state.showAdvancedFilters) 
                ? (<span className="visible-xs">{ '(' + Object.keys(this.props.query).length + ' active)'}</span>) 
                : null
            }            
            </span>
          </Grid>
...
//      }
  ...
  noOfHiddenActiveFilters : function(){

    return _.intersection (
        _.pluck(_.flatten(_.pluck(_.flatten(_.pluck(_.filter(this.props.filterRows,{bydefault:false}),'groups')),'filters')),'by'), //hiddenFilterIds
        Object.keys(this.props.query) //activeFilterIds
      ).length 
  },
