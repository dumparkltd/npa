...
var PrintStore = Fluxxor.createStore({
    ...
    function (actionStore, issueStore, groupStore, agencyStore, pageStore) {
      ...
      var docDefinition = {
        footer: function(currentPage, pageCount) {
          if(currentPage > categoryStartPage && currentPage < actionStartPage){
            return {
          ...
          if(currentPage > actionStartPage){
            return {
  ...
  getCategory: function (payload, category, relatedEntities) {
    ...
    var categoryContent = {
      content: [
        ...
        this.getCategoryContent(category, payload.type, relatedEntities, 3).category,
  ...
  getCategoryContent: function (category, type, relatedEntities, indexStart) {
    var relatedEntitiesIndex = _.map(relatedEntities, function (value, key) {
      var tableOfContents = {
        columns: [
          {width: '80%', text: value.title},
          {width: '20%', text: 'B' + indexStart.toString()}
        ]
      }
      indexStart += _.isEmpty(value.printnoofpages) ? 1 : +value.printnoofpages
    ...
    return {category: categoryContent, pageLength: indexStart}
  },
  ...
  getAll: function (payload, relatedEntities, allAgencies, allGroups, allIssues) {
    var relatedGroupEntities = _.map(allGroups, function (value, key) {
      ...
      var category = this.getCategoryContent(value, 'group', relatedGroupActions, 1)
      actionIndex = parseInt(category.pageLength)
    ...
    var relatedIssueEntities = _.map(allIssues, function (value, key) {
      ...
      category = this.getCategoryContent(value, 'issue', relatedIssueActions, actionIndex)
      actionIndex = parseInt(category.pageLength)
      return category.category
    }, this)
    ...
    var relatedAgencyEntities = _.map(allAgencies, function (value, key) {
      ...
      category = this.getCategoryContent(value, 'agency', relatedAgencyActions, actionIndex)
      actionIndex = parseInt(category.pageLength)
      return category.category
    }, this)
