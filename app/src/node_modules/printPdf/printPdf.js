...
module.exports = function (report, reportId, callback) {  
  ...
  switch (report.type) {
    ...
    case 'action' :  
      ...
      content.push({
        stack: titleLayout(report.titleData,report.type,report.sections,elements),
        pageBreak:'after',
        footerMode: '0' 
      })
      content.push({ 
    ...
    case 'agency' :            
      ...
      content.push({ 
        stack: categoryLayout(report.data.categoryData, report.type, false, elements),        
        pageBreak:'after',
        footerMode: 'A',         
      })     
      content.push({ 
      ...
      _.each(report.data.actionsDataList,function(actionData,i){
        content.push({ 
          stack: actionLayout(actionData, elements),
    ...
    case 'all' :     
      ...
      _.each(report.data.categoriesDataList,function(categories,type){
        _.each(categories,function(catData){
          content.push({ 
            stack: categoryLayout(catData, type, false, elements),        
            pageBreak:'after',
            footerMode: 'A',         
          })     
          content.push({ 
      ...
      if (footerMode === '0') {
        return report.titleData.url.length < 130 ? {
           stack : [
            {
              text: [
                {text: 'Report URL: '}, {text: report.titleData.url}
              ],
