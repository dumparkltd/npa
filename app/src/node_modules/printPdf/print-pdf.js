...
var blobFeatureCheck = require('./blob-feature-check')
...
module.exports = function (report, reportId, callback) {  
    ...
    case 'all' :     
      ...
      _.each(report.data.categoriesDataList,function(categories,type){
        _.each(categories,function(catData){
          content.push({ 
            stack: categoryLayout(catData, type, true),        
            pageBreak:'after',
            footerMode: 'A',         
          })     
          content.push({ 
  ...
  var docDefinition = {
    ...
    // footerMode : section
    footer: function (currentPage, pageCount, pageSize, footerMode) {
      if (footerMode === '0') {
      ...
      $script('js/pdfmake.min.js',function(){
        $script('js/vfs_fonts.js', function(){              
          ...
          try {
            ...
            blobFeatureCheck(function(result){
              if (result) {
                ...
                callback({id:reportId,status:{code:'error_blob',message:'We are sorry, your browser is not yet supported',level:'error'}})                            
              }
            })            

          } catch (e) {
            callback({id:reportId,status:{code:'error_report',message:'We are sorry, your report could not be generated',level:'error'}})            
